...
//             <li>
//               Multiple questions are displayed simultaneously (Double Layout).
//             </li>
//             <li>Once submitted, you cannot modify your answers.</li>
//             <li>Certificate unlocks only if you score 70% or higher.</li>
//             <li>
//               Do not refresh the page; it may result in an automatic submission.
//             </li>
//           </ul>
...
//           <div
//             className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center mb-6 shadow-2xl ${passed ? "bg-emerald-500 text-white shadow-emerald-200" : "bg-red-500 text-white shadow-red-200"}`}
//           >
//             {passed ? (
...
//             Correct Answers:{" "}
//             {quizData?.attempts[quizData?.attempts.length - 1]?.score || 0} /{" "}
//             {totalQuestions}
//           </p>
...
//               className="py-4 bg-gray-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-black transition-all shadow-xl"
//             >
...
//                 Maximum attempts reached. Contact support if needed.
//               </p>
//             )}
//           </div>
//         </div>
...
//             <div
//               key={qIndex}
//               className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300"
//             >
//               <div className="flex justify-between items-center mb-4">
//                 <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest underline decoration-2 underline-offset-4">
//                   Question {qIndex + 1}
//                 </span>
...
//                 )}
//               </div>
//               <h4 className="text-lg font-bold text-gray-800 mb-6 leading-tight h-auto md:h-14">
//                 {q.question}
//               </h4>
...
//                     {opt}
//                     <div
//                       className={`h-4 w-4 rounded-full border-2 ${selectedOptions[qIndex] === optIndex ? "bg-white border-white" : "border-gray-200"}`}
//                     ></div>
//                   </button>
//                 ))}
//               </div>
...
//             <div className="bg-orange-100 text-orange-600 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 transform rotate-12">
...
//               You have answered {Object.keys(selectedOptions).length} of{" "}
//               {totalQuestions} questions. Do you want to submit?
//             </p>
...
//                 className="w-full py-3 bg-gray-50 rounded-2xl font-bold text-gray-400 hover:text-gray-600 transition-all"
//               >
//                 Keep Reviewing
//               </button>
//             </div>
...
const QuizAttempt = () => {
  ...
  // ✅ FIX 1: quizData.attempts sirf current user ke filtered hain (backend already filter karta hai)
  const attemptsLeft = quizData?.attempts
    ? Math.max(0, MAX_ATTEMPTS - quizData.attempts.length)
    : MAX_ATTEMPTS;
  ...
  // ✅ FIX 2: handleSubmit ko useCallback mein wrap kiya — stale closure fix
  const handleSubmit = useCallback(async () => {
    try {
      ...
      // ✅ FIX 3: result.data nahi, directly result mein score/percentage hai
      // kyunki quizAPI.js mein .then(res => res.data) already hai
      setScore(result.score);
      setPercentage(result.percentage);
      setIsSubmitted(true);
      setShowConfirm(false);

      // ✅ FIX 4: result.data.quiz nahi — result.quiz hai
      if (result.quiz) dispatch(setQuizData(result.quiz));

    } catch (err) {
      toast.error(err.response?.data?.message || "Submission failed");
    }
  }, [selectedOptions, quizData, dispatch]);

  // ✅ FIX 5: handleSubmit dependency properly add ki
  useEffect(() => {
    if (quizStarted && timeLeft > 0 && !isSubmitted) {
      const timer = setTimeout(() => setTimeLeft((prev) => prev - 1), 1000);
      return () => clearTimeout(timer);
    } else if (timeLeft === 0 && quizStarted && !isSubmitted) {
      handleSubmit();
    }
  }, [timeLeft, isSubmitted, quizStarted, handleSubmit]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 font-sans">
        <div className="max-w-2xl w-full bg-white rounded-[2.5rem] shadow-xl p-10 border border-gray-100">
          ...
          <ul className="space-y-3 text-gray-500 text-sm mb-10 list-disc px-5 font-medium">
            <li>Multiple questions are displayed simultaneously.</li>
            <li>Once submitted, you cannot modify your answers.</li>
            <li>Certificate unlocks only if you score 70% or higher.</li>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-6">
        <div className="max-w-lg w-full text-center">
          ...
          {/* ✅ FIX 6: score state use kiya, quizData.attempts pe depend nahi kiya */}
          <p className="text-gray-400 font-bold mb-2 uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 gap-8">
          {quizData?.questions?.map((q, qIndex) => (
            <div key={qIndex} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
              ...
              <h4 className="text-lg font-bold text-gray-800 mb-6 leading-tight">
                {q.question}
              </h4>
              <div className="space-y-3">
                {q.options.map((opt, optIndex) => (
                  ...
                  >
                    {opt}
                    <div className={`h-4 w-4 rounded-full border-2 flex-shrink-0 ${selectedOptions[qIndex] === optIndex ? "bg-white border-white" : "border-gray-200"}`} />
                  </button>
                ))}
              </div>
            </div>
