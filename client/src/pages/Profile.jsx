import { 
  User, BookOpen, Award, Settings, LogOut, Mail, 
  ChevronRight, Home, Shield, Bell
} from 'lucide-react';
...
const [formData, setFormData] = useState({
  ...
  const handleSave = () => {
  // Yahan aap apni Redux action ya API call daal sakte hain
  ...
  return (
    <div className="w-full min-h-screen bg-white font-sans selection:bg-indigo-100">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-6 flex items-center gap-3 text-sm font-medium">
        <Link to="/" className="text-slate-400 hover:text-indigo-600 transition-colors flex items-center gap-1.5">
          <Home size={16} /> Home
        </Link>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
       ...
       {/* Left Column: Modern Minimal Sidebar */}
      <aside className="lg:col-span-4 space-y-6">
      {/* Profile Card */}
      <div className="relative overflow-hidden p-6 rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/40 flex flex-col items-center text-center">
  {/* Decorative Blur - Compact version */}
  <div className="absolute -top-12 -right-12 w-32 h-32 bg-indigo-50 rounded-full blur-3xl opacity-60" />
  
  {/* Profile Image - Size reduced from w-28 to w-20 */}
  <div className="relative mb-4 group cursor-pointer">
    <div className="absolute inset-0 bg-indigo-500 rounded-full blur-xl opacity-10 animate-pulse" />
    <div className="relative p-0.5 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 transition-transform group-hover:scale-105">
      {userData?.photourl ? (
        <img 
      ...
      {/* Camera Icon - Now inside 'group' container */}
      <label className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
  ...
  <div className="relative">
    ...
    <p className="text-slate-500 text-[12px] font-medium mt-3 px-1 leading-tight line-clamp-2 italic">
      "Software engineer passionate about backend architecture."
    </p>
  </div>
  ...
  <div className="mt-5 w-full grid grid-cols-3 divide-x divide-slate-100 bg-slate-50/50 rounded-2xl py-2 border border-slate-100">
    <div className="flex flex-col items-center">
      <span className="text-[9px] text-slate-400 uppercase font-bold tracking-tighter">Enrolled</span>
      <span className="text-sm font-black text-slate-700">05</span>
    </div>
    <div className="flex flex-col items-center">
      <span className="text-[9px] text-slate-400 uppercase font-bold tracking-tighter">Done</span>
      <span className="text-sm font-black text-emerald-600">04</span>
    </div>
    <div className="flex flex-col items-center">
      <span className="text-[9px] text-slate-400 uppercase font-bold tracking-tighter">Certis</span>
      ...
      <nav className="bg-white/60 backdrop-blur-md p-3 rounded-4xl border border-slate-100 space-y-1">
        ...
        <div className="pt-3 mt-3 border-t border-slate-100">
          <button className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-rose-500 hover:bg-rose-50 transition-all duration-300 font-bold text-sm group">
        ...
        <div className="lg:col-span-8">
          <div className="min-h-125">
              ...
              <div className="flex items-center justify-between border-b pb-4">
                ...
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                    isEditing ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full mt-1 px-4 py-3 rounded-xl border transition-all ${
                      isEditing ? 'border-indigo-500 bg-white shadow-sm' : 'border-transparent bg-slate-50 opacity-70'
                ...
                {/* Email (Locked) */}
                <div>
                  <label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Email (Read Only)</label>
              ...
              {isEditing && (
                <button onClick={() => setIsEditing(false)} className="text-sm text-slate-400 hover:text-red-500 font-medium ml-1">
                  Cancel changes
                </button>
              )}
              </div>
            ...
            {activeTab === 'learning' && (
               <div className="animate-in fade-in duration-500">
                 <h3 className="text-3xl font-black text-slate-900 mb-8">Learning Path</h3>
               </div>
            )}
          </div>
        </div>
