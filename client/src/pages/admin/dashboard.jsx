...
const AdminDashboard = () => {
  ...
  const pieData = [
    ...
    {
      name: "Educators",
      value: analytics?.totalEducators || 0,
      color: "#10b981",
    },
  ];

  const hasData =
    (analytics?.totalStudents || 0) + (analytics?.totalEducators || 0) > 0;
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              LMS Platform Performance Overview
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-[#161636]/40 border border-white/10 rounded-4xl p-8 backdrop-blur-md">
            <h3 className="text-base font-bold text-white mb-8 flex items-center gap-2">
              <TrendingUp className="text-indigo-400" size={18} /> Platform
              Growth (Weekly)
            </h3>
          ...
          <div className="bg-[#161636]/40 border border-white/10 rounded-4xl p-8 backdrop-blur-md">
            <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest mb-8 text-center">
              User Split
            </h3>
              ...
              {hasData ? (
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    ...
                    >
                      {pieData.map((e, i) => (
                        <Cell key={i} fill={e.color} cornerRadius={10} />
                      ))}
                    </Pie>
                    <Tooltip
                      contentStyle={{
