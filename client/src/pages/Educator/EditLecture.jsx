  ...
  useEffect(() => {
    if (selectedLecture) {
      ...
      if (selectedLecture.videoUrl) {
        setVideoPreview(selectedLecture.videoUrl);
      }
    }
  }, [selectedLecture]);
          ...
          >
            <div className="bg-white p-6 rounded-4xl shadow-xl shadow-slate-200/50 border border-slate-100">
                ...
                <AnimatePresence mode="wait">
                  {videoPreview ? (
                    ...
                    >
                      <FiUpload
                        size={32}
                        className="text-slate-600 mb-2 opacity-80"
                      />
              ...
              <label className="block w-full text-center py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl cursor-pointer transition-all active:scale-[0.98] font-bold text-xs mb-6 shadow-lg shadow-indigo-200">
                {videoUrl ? "Change Video File" : "Upload Video"}
                <input
                  type="file"
                  accept="video/*"
                  className="hidden"
              ...
              <div className="space-y-3">
                ...
                <div
                  ...
                  className={`flex items-center justify-between p-4 rounded-xl cursor-pointer transition-all border ${isPreviewFree ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-100"}`}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-lg ${isPreviewFree ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                    >
                      {isPreviewFree ? (
                        <FiUnlock size={16} />
                      ) : (
                    ...
                    <div>
                      <p
                        className={`text-xs font-bold ${isPreviewFree ? "text-emerald-700" : "text-slate-700"}`}
                      >
                        Free Preview
                      </p>
                  ...
                  <div
                    className={`w-10 h-5 rounded-full p-1 transition-colors ${isPreviewFree ? "bg-emerald-500" : "bg-slate-300"}`}
                  >
                    <div
                      className={`bg-white w-3 h-3 rounded-full transition-transform ${isPreviewFree ? "translate-x-5" : "translate-x-0"}`}
                    ></div>
                  </div>
                </div>
              </div>
