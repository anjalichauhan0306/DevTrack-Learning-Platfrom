import { 
  User, BookOpen, Award, Settings, LogOut, 
  ChevronRight, Home
} from 'lucide-react';
...
const Profile = () => {
  ...
  const handleEditProfile = async() => {
    ...
    try {
        ...
        formData.append("description", description || userData?.description || "");
      ...
      <div className="relative mb-4 group cursor-pointer">
        ...
        <div className="relative p-0.5 rounded-full bg-linear-to-tr from-indigo-500 to-purple-500 transition-transform group-hover:scale-105">
          ...
          <label className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full opacity-0 group-hover:opacity-100 transition-all cursor-pointer">
            ...
            <input type="file" className="hidden" onChange={(e)=>setPhotourl(e.target.files[0])} accept='image/*'
            value={photoUrl}
    ...
    <div className="flex flex-col items-center">
      ...
      <nav className="bg-white/60 backdrop-blur-md p-3 rounded-4xl border border-slate-100 space-y-1">
        ...
        <div className="pt-3 mt-3 border-t border-slate-100">
          <button onClick={handleLogout} className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-rose-500 hover:bg-rose-50 transition-all duration-300 font-bold text-sm group">
          ...
          <div className="min-h-125">
              ...
              <div className="flex items-center justify-between border-b pb-4">
                ...
                <button 
                  ...
                  {loading ? ( <> <ClipLoader size={30} color='white' /> <span>Saving...</span> </> ) : (isEditing ? 'Save' : 'Edit Profile')}
                </button>
              </div>
